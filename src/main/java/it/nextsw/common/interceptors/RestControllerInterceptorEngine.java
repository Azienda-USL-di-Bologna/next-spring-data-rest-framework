...
//        fillInterceptorsCache();
        List<RestControllerInterceptor> interceptors = interceptorsMap.get(entityReflectionUtils.getEntityFromProxyClass(entityClass).getName());
    ...
    public Object executeAfterSelectQueryInterceptor(Object entity, List<Object> entities, Class entityClass, HttpServletRequest request, Map<String, String> additionalData) throws ClassNotFoundException, InterceptorException, EntityReflectionException {
...
//        fillInterceptorsCache();
        List<RestControllerInterceptor> interceptors = interceptorsMap.get(entityReflectionUtils.getEntityFromProxyObject(entity).getName());
        if (interceptors != null) {
            for (RestControllerInterceptor interceptor : interceptors) {
